...
const SINGLE_WORD_ADVERBS = new Set();
const MULTI_WORD_ADVERBS = [];
...
function normalizeWord(word) {
    ...
    function tryMatch(candidate) {
        if (candidate && SINGLE_WORD_ADVERBS.has(candidate.toLowerCase())) {
            return candidate;
        }
        return null;
    }
    function stripRecursively(candidate, depth = 0) {
        if (depth > 10 || !candidate)
            return null;
...
function checkAdverbs(text) {
    ...
    for (const multiAdverb of MULTI_WORD_ADVERBS) {
        ...
        while ((match = regex.exec(textLower)) !== null) {
            ...
            const isOverlapping = foundPositions.some(([pos, posEnd]) => overlaps(start, end, pos, posEnd));
            if (!isOverlapping) {
    ...
    for (const word of words) {
        ...
        position = wordEnd;
        const isOverlapping = foundPositions.some(([pos, posEnd]) => overlaps(wordStart, wordEnd, pos, posEnd));
        if (!isOverlapping) {
...
export default function createServer({ config }) {
    ...
    server.registerTool("check_adverbs", {
        title: "Check Adverbs",
